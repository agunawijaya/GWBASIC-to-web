...
| **32** | **Penutup — selesai**, Σ 66. **`docs/whatmonf.md`** ditulis, dokumen terakhir yang belum ada: empat baris, 117 bita, program TERPENDEK di koleksi dan satu-satunya yang tidak jadi aplikasi — bukan karena membosankan, tapi karena ia tidak mencetak apa pun. Baris 30-40 menerjemahkan mode video BIOS jadi alamat penyangga layar, dan terjemahannya **tertukar persis**: mode 2/3 (teks berwarna CGA) diberi `&HB000` yang alamat monokrom, mode 7 (monokrom) diberi `&HB800` yang alamat berwarna. Metodenya sendiri sah — membaca `0040:0049` memang cara yang benar untuk mengetahui mode video; yang salah cuma dua konstanta yang dipertukarkan. Akibatnya tidak seperti cacat biasa yang 'jalan di mesin saya': ia salah di **KEDUA** jenis mesin, karena PC berkartu CGA tidak punya memori di `B000` dan PC monokrom tidak punya di `B800` — tulisannya lenyap di dua-duanya, tanpa galat, cuma layar diam. Pembandingnya ada di disket yang SAMA: `SUB.BAS` menanyakan hal yang sama enam kali lewat byte perlengkapan (`IF (PEEK(1040) AND 48)=48 THEN DEF SEG=45056 ELSE 47104`) dan **benar setiap kali**. Disapu seluruh koleksi: **empat belas** program menyentuh byte video BIOS, **tiga belas** memakai byte perlengkapan `0040:0010`, dan **tepat satu** memakai byte mode `0040:0049` — yang satu itu WHATMONF, dan yang menyendiri juga yang keliru. Temuan terakhir: sesudah baris 40 programnya BERHENTI dan `SCRN` tidak pernah dibaca — nol `PRINT`, nol `POKE`. Jadi ia bukan program melainkan **cuplikan** yang diterbitkan majalah *What Micro?* untuk disalin, dan cuplikan yang salah lebih berbahaya daripada program yang salah: ia ditempelkan ke tengah kode orang lain, di mana kegagalannya muncul jauh dari sumbernya. Ini salah satu dari dua perbaikan yang disebut keputusan (c) §9; karena WHATMONF tidak punya aplikasi, perbaikannya hidup di dokumennya, bukan di kode. **SILANG-RUJUK di peluncur:** sampai sesi ini kartu hanya menaut ke aplikasinya — dokumen arsitekturnya cuma bisa ditemukan dari dalam halaman permainan, dan enam belas entri yang tidak jadi aplikasi tidak punya pintu masuk sama sekali. Sekarang **74 kartu** menaut langsung ke dokumennya lewat medan `dok` di katalog, bukan tebakan `docs/<id>.md` — karena keempat varian blackjack berbagi satu `blackjack.md` dan enam belas entri memang tidak punya dokumen; menebak akan menghasilkan dua puluh tautan rusak yang di `file://` tidak bisa diperiksa lebih dulu. Diverifikasi: 74 tautan, keempat varian blackjack menunjuk `blackjack.md`, WHATMONF kini punya pintu masuk, label dwibahasa, klik tautan dokumen tidak ikut membuka aplikasinya, dan nihil luber mendatar di enam lebar untuk kedua bahasa | — | 66 | 🚪 **GERBANG AKHIR** |
